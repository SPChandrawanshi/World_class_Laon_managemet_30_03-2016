...
import { LogOut, Edit2, Phone, Mail, Building, User, Shield, Camera } from 'lucide-react';
...
export default function LenderProfile() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Profile Card */}
      <div className="bg-[#161b22] rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="h-24 bg-gradient-to-r from-blue-700 to-blue-600" />
        <div className="px-5 pb-5">
          <div className="flex items-end justify-between -mt-10 mb-4">
            <div className="relative w-fit">
              <div className="w-20 h-20 rounded-2xl bg-blue-100 border-4 border-white shadow-lg flex items-center justify-center text-blue-700 font-bold text-2xl overflow-hidden relative group">
                {user?.initials || 'LP'}
      ...
      <div className="bg-[#161b22] rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Money Lender's License</h3>
          <span className="px-2 py-0.5 rounded-md bg-green-100 text-green-700 text-[10px] font-bold">Verified</span>
        </div>
        <label className="flex items-center justify-center gap-2 w-full py-3.5 border-2 border-dashed border-[#30363d] rounded-xl bg-[#0d1117] cursor-pointer hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors text-gray-600 font-bold text-sm">
      ...
      {/* Account Status */}
      <div className="bg-[#161b22] rounded-2xl border border-gray-100 shadow-sm p-4">
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Account Status</h3>
      ...
      <div className="flex justify-center pt-2">
        <button
          ...
          className="flex items-center justify-center gap-2 px-6 py-2 rounded-xl border border-red-200 text-red-600 font-bold text-xs hover:bg-red-50 transition-colors active:scale-95 bg-[#161b22]"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={key} className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider px-1">{label}</label>
                <input type={type} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-[#0d1117] border border-[#30363d] rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
              </div>
            ))}
          </div>

          <div className="pt-4 border-t border-gray-100">
            <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-4 px-1">Security & Password</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
      ...
      {toastMsg && (
        <div className="fixed bottom-24 right-6 left-6 md:left-auto md:w-80 z-[60] bg-blue-600 text-white px-5 py-3.5 rounded-2xl shadow-xl shadow-blue-600/30 flex items-center gap-3 font-bold text-sm" style={{ animation: 'slideUp 0.3s ease' }}>
          <div className="w-6 h-6 rounded-full bg-[#161b22]/20 flex items-center justify-center text-xs flex-shrink-0">ℹ</div>
          {toastMsg}
        </div>
      )}
