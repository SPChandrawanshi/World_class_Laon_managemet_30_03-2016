...
import {
  ...
  User, LogOut, Menu, X, ChevronRight, Calendar, Gift,
  Bell, DollarSign
} from 'lucide-react';
      ...
      <div className={`flex items-center gap-3 px-6 py-6 border-b border-[#30363d] ${isCollapsed ? 'flex-col gap-6 justify-center' : ''}`}>
        ...
        {!isCollapsed ? (
          <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <p className="text-[10px] text-blue-500 font-bold uppercase tracking-wider mt-1 opacity-80 leading-none">Staff Portal</p>
          </div>
        ) : null}
      ...
      {!isCollapsed && (
        <div className="mx-4 mt-6 mb-3 p-4 rounded-3xl bg-[#0d1117] border border-[#30363d] flex items-center gap-4 group transition-all hover:bg-[#1c2128] animate-in fade-in zoom-in-95 duration-300 shadow-sm">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[#8b949e] text-[9px] font-bold uppercase tracking-wider">Certified Staff</p>
          </div>
        </div>
      )}
          ...
          return (
            <Link key={path} to={path} onClick={onLinkClick}
              ...
              <div className="flex flex-col min-w-0 flex-1">
                ...
                {!isCollapsed && <span className="text-[7px] text-white/40 font-bold uppercase tracking-widest mt-0.5 leading-none">{NAV.find(n => n.path === path)?.detail}</span>}
              </div>
            </Link>
          );
        })}
      ...
      <div className="flex-1 flex flex-col min-w-0 ">
        ...
        <header className="flex items-center justify-between px-5 md:px-10 py-1 bg-[#161b22]/95 border-b border-[#30363d] sticky top-0 z-40 shadow-sm backdrop-blur-md">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <Link to="/staff/profile" className="flex items-center gap-2 p-1 md:p-1.5 pr-4 md:pr-5 rounded-2xl bg-[#0d1117] border border-[#30363d] hover:border-blue-500/50 transition-all active:scale-95 group shadow-sm">
              ...
              <div className="hidden sm:flex flex-col min-w-0">
                ...
                <span className="text-[8px] font-bold text-blue-500 uppercase tracking-wider leading-none">Staff</span>
              </div>
            </Link>
          </div>
        </header>
