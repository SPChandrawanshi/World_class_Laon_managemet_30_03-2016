...
import {
  LayoutDashboard, Users, DollarSign, History,
  User, LogOut, Menu, X, ChevronRight, Search, Briefcase
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { THEME } from '../theme';

const NAV = [
  { path: '/agent/dashboard', label: 'Dashboard', detail: 'Overview', icon: LayoutDashboard },
  { path: '/agent/clients', label: 'Clients', detail: 'Referred borrowers', icon: Users },
  { path: '/agent/earnings', label: 'Earnings', detail: 'Commission', icon: DollarSign },
  { path: '/agent/payments', label: 'Payments', detail: 'Commission payout', icon: History }, // Using History icon for payout
  { path: '/agent/notifications', label: 'Notifications', detail: 'Alerts', icon: Briefcase }, // Using Briefcase for generic alerts/notifications
];

const C = THEME.role.agent;
  ...
  const SidebarContent = ({ onLinkClick, isCollapsed = false }) => (
    <>
      <div className={`flex items-center gap-3 px-6 py-6 border-b border-[#30363d] ${isCollapsed ? 'flex-col gap-6 justify-center' : ''}`}>
        <div className="w-16 h-16 flex items-center justify-center bg-transparent overflow-hidden flex-shrink-0"><img src={logo} alt="Logo" className="w-full h-full object-contain" /></div>
        {!isCollapsed ? (
          <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-left-2 duration-300">
            <h1 className="text-lg font-bold text-white tracking-tight uppercase leading-none">Global <span className="text-blue-600">Loan</span></h1>
            <p className="text-[10px] text-blue-600 font-bold uppercase tracking-wider mt-1 opacity-80 leading-none">Agent Portal</p>
          </div>
        ) : null}
      ...
      {!isCollapsed && (
        <div className="mx-4 mt-6 mb-3 p-4 rounded-3xl bg-[#0d1117] border border-[#30363d] flex items-center gap-4 group transition-all hover:bg-[#1c2128] animate-in fade-in zoom-in-95 duration-300 shadow-sm">
          ...
          <div className="min-w-0">
            <p className="text-[#e6edf3] text-xs font-bold uppercase tracking-tight truncate">{user?.name || 'Agent'}</p>
            <p className="text-[#8b949e] text-[9px] font-bold uppercase tracking-wider">External Partner</p>
          </div>
        </div>
      )}
      ...
      <nav className={`flex-1 px-3 py-4 space-y-2 overflow-y-auto min-h-0 custom-scrollbar mt-2 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
        {NAV.map(({ path, label, detail, icon: Icon }) => {
            ...
            <Link key={path} to={path} onClick={onLinkClick}
              ...
              {!isCollapsed && (
                <div className="flex flex-col text-left">
                  ...
                  <span className={`text-[8px] font-bold uppercase tracking-widest opacity-60 ${active ? 'text-white/80' : 'text-slate-500'}`}>{detail}</span>
                </div>
              )}
        ...
        <header className="flex items-center justify-between px-5 md:px-10 py-1 bg-[#161b22]/95 border-b border-[#30363d] sticky top-0 z-40 shadow-sm backdrop-blur-md">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <div className="hidden sm:flex relative group md:w-80">
              ...
              <input 
                type="text" 
                placeholder="Search Clients / Earnings..." 
                className="pl-11 pr-4 py-3 bg-[#0d1117] border border-[#30363d] rounded-2xl text-[10px] font-bold uppercase tracking-wider text-[#e6edf3] placeholder:text-[#6e7681] focus:border-blue-600 focus:bg-[#1c2128] outline-none w-full transition-all shadow-sm"
              />
            </div>
            <Link to="/agent/profile" className="flex items-center gap-2 p-1 md:p-1.5 pr-4 md:pr-5 rounded-2xl bg-[#0d1117] border border-[#30363d] hover:border-blue-500/50 transition-all active:scale-95 group shadow-sm">
              ...
              <div className="hidden sm:flex flex-col min-w-0">
                ...
                <span className="text-[8px] font-bold text-blue-500 uppercase tracking-wider leading-none">Agent Partner</span>
              </div>
            </Link>
          </div>
        </header>
            ...
            {NAV.slice(0, 4).map(({ path, label, icon: Icon }) => {
              ...
              return (
                <Link key={path} to={path}
                  ...
                  <span className={`text-[8px] font-bold mt-1 uppercase tracking-wider truncate w-full text-center px-1 ${active ? 'opacity-100' : 'opacity-60'}`}>{displayLabel}</span>
                </Link>
              );
            })}
          </div>
