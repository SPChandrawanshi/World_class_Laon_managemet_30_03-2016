...
import {
  LayoutDashboard, CreditCard, User, LogOut, Menu, X, ChevronRight, Search, Gift,
  PlusCircle, Image as ImageIcon, Calendar, Bell, DollarSign
} from 'lucide-react';
...
const NAV = [
  ...
  { path: '/app/apply', label: 'Apply Loan', icon: PlusCircle },
  { path: '/app/loans', label: 'My Loans', icon: CreditCard },
  { path: '/app/payments', label: 'Payments', icon: DollarSign },
  { path: '/app/collateral', label: 'Collateral', icon: ImageIcon },
      ...
      <div className={`flex items-center gap-3 px-6 py-6 border-b border-[#30363d] ${isCollapsed ? 'flex-col gap-6 justify-center' : ''}`}>
        ...
        {!isCollapsed ? (
          <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <p className="text-[10px] text-blue-600 font-bold uppercase tracking-wider mt-1 opacity-80 leading-none">Borrower Portal</p>
          </div>
        ) : null}

        {/* Desktop Toggle Button inside Sidebar */}
        <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="relative z-10 cursor-pointer hidden lg:flex w-8 h-8 items-center justify-center rounded-xl bg-[#161b22]/10 text-white hover:bg-blue-600 hover:text-white transition-all border border-[#30363d] active:scale-95 group"
        >
      ...
      {!isCollapsed && (
        <div className="mx-4 mt-6 mb-3 p-4 rounded-3xl bg-[#0d1117] border border-[#30363d] flex items-center gap-4 group transition-all hover:bg-[#1c2128] animate-in fade-in zoom-in-95 duration-300 shadow-sm">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[#8b949e] text-[9px] font-bold uppercase tracking-wider">{user?.nrc || 'Borrower'}</p>
          </div>
        </div>
      )}
        ...
        <header className="flex items-center justify-between px-5 md:px-10 py-1 bg-[#161b22]/95 border-b border-[#30363d] sticky top-0 z-40 shadow-sm backdrop-blur-md">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <div className="hidden sm:flex relative group md:w-80">
              <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6e7681] group-focus-within:text-blue-600 transition-colors" />
              <input 
                type="text" 
            ...
            <Link to="/app/profile" className="flex items-center gap-2 p-1 md:p-1.5 pr-4 md:pr-5 rounded-2xl bg-[#0d1117] border border-[#30363d] hover:border-blue-500/50 transition-all active:scale-95 group shadow-sm">
              <div className="w-7.5 h-7.5 rounded-lg bg-teal-600 flex items-center justify-center text-white font-bold text-xs shadow-xl shadow-teal-600/20 group-hover:bg-teal-500 transition-colors">
                {user?.initials || 'BW'}
              </div>
              <div className="hidden sm:flex flex-col min-w-0">
                ...
                <span className="text-[8px] font-bold text-teal-600 uppercase tracking-wider leading-none">Verified</span>
              </div>
            </Link>
          </div>
        </header>
